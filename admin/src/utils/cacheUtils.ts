...
export function saveToCache<T>(key: string, data: T, date: string): void {
  try {
    ...
    if (error.name === 'QuotaExceededError' || error.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
      devLogger.warn('localStorage 空间不足，正在尝试清空旧缓存...');
      clearAllCache();
      // 尝试再次保存
      try {
        const cacheData: CacheData<T> = {
        ...
        devLogger.error('重试保存缓存失败:', retryError);
      }
    } else {
      devLogger.error('保存缓存失败:', error);
    }
  }
}
...
export function clearAllCache(): void {
  try {
    const keys = Object.keys(localStorage);
    keys.forEach(key => {
      if (key.startsWith(CACHE_PREFIX) || key === WORKFLOW_RUN_STORAGE_KEY) {
        localStorage.removeItem(key);
      }
    });
  } catch (error) {
    devLogger.error('清除所有缓存失败:', error);
  }
}

/** 清除指定日期的生成页缓存 */
export function clearGenerationCacheForDate(date: string): void {
  clearCache(CACHE_KEYS.GENERATION_RESULT, date);
  clearCache(CACHE_KEYS.GENERATION_SELECTED_IDS, date);
  clearCache(CACHE_KEYS.GENERATION_SELECTED_ITEMS, date);
}
...
export function clearExpiredCache(): void {
  try {
    ...
    devLogger.error('清除过期缓存失败:', error);
  }
}

/** 工作流运行会话（与 CACHE_PREFIX 键名一致，供 clearAllCache 一并清除） */
export const WORKFLOW_RUN_STORAGE_KEY = 'ai_insight_daily_workflow_run_session_v1';
