...
const WechatPublishModal: React.FC<WechatPublishModalProps> = ({ date, content, onClose, onSuccess, onError }) => {
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      try {
        ...
        devLogger.error('Failed to load agents/workflows/tools for cover generation:', e);
      }
    };
  ...
  const handleGenerateCover = async () => {
    ...
    try {
      ...
      if (res.isHtml || res.html) {
        ...
        if (htmlCaptureRef.current) {
          ...
          try {
            ...
            devLogger.error('Failed to capture HTML as image:', screenshotError);
        ...
        if (imageUrl) {
          if (urlToMediaIdMap[imageUrl]) {
            ...
            try {
              ...
              devLogger.error('Upload to WeChat failed:', uploadError);
      ...
      devLogger.error('Generate cover failed:', error);
  ...
  const handleProcessContent = async () => {
    ...
    try {
      ...
      if (isHtmlResult && publishAsImageText) {
        ...
        if (htmlCaptureRef.current) {
          ...
          try {
            ...
            devLogger.error('Failed to capture HTML content as image:', screenshotError);
      ...
      devLogger.error('Process content failed:', error);
  ...
  const handleViewImage = (url: string) => {
    ...
    if (finalUrl.startsWith('data:')) {
      try {
        ...
        devLogger.error('Failed to open base64 image:', e);
        // 回退到简单方式
  ...
  const robustUpload = async (url: string) => {
    ...
    try {
      ...
      originalError = error;
      devLogger.warn('Regular upload failed, trying frontend capture fallback...', error);
      // 给用户一个提示，说明正在尝试备选方案
      ...
      try {
        ...
        if (img.complete && img.naturalWidth > 0) {
          ...
          if (ctx) {
            ...
            if (dataUrl) {
              devLogger.info('Successfully captured image from frontend DOM, uploading as base64...');
          ...
          devLogger.warn('Image DOM element found but not fully loaded or zero size.');
        }
      } catch (frontendError: any) {
        devLogger.error('Frontend capture process failed:', frontendError);
      }
    }
                      ...
                      onClick={async () => {
                        ...
                        if (urlToMediaIdMap[url]) {
                          ...
                          try {
                            ...
                            devLogger.error('Upload to WeChat failed:', error);
