...
export interface KnowledgeQueryMeta {
  ...
  plannerStages?: Array<Record<string, any>>;
  retrievalStages?: Array<Record<string, any>>;
  durationMs?: number;
  sourceCount?: number;
}
...
export const knowledgeService = {
  ...
  uploadDocument: async (categoryId: string, file: File): Promise<KnowledgeWriteResult> => {
  ...
  updateDocumentContent: (id: string, content: string): Promise<KnowledgeWriteResult> =>
  ...
  queryKnowledge: (
    ...
    request('/api/kb/query', {
      method: 'POST',
      body: JSON.stringify({ query, categoryIds, documentIds, limit })
    }),
