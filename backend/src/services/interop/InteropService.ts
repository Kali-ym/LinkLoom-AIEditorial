...
import { maskSettingsForResponse, mergeSettingsUpdate } from '../settingsSecurity.js';
...
export class InteropService {
  ...
  async createApiKey(apiKeyData: {
    ...
    const verificationToken =
      apiKeyData.status === 'pending' ? crypto.randomBytes(16).toString('hex') : undefined;
  ...
  async registerPendingKey(
    name: string,
    fingerprint: string
  ): Promise<{ id: string; key: string; verificationUrl: string }> {
    // 检查指纹是否已存在
  ...
  async updateApiKey(id: string, data: { name?: string; status?: string }): Promise<void> {
    ...
    if (data.name) {
      // 需要先获取原记录，因为 store.saveApiKey 使用 ON CONFLICT 更新
  ...
  async deleteAgent(id: string): Promise<void> {
    const workflows = await this.store.listWorkflows();
    const refs = findWorkflowsReferencingAgent(id, workflows);
    if (refs.length > 0) {
      const summary = refs.map((wf) => `${wf.name}（${wf.id}）`).join('、');
      throw new Error(`该智能体正被工作流引用：${summary}`);
    }
  ...
  async getSettings(): Promise<any> {
    return maskSettingsForResponse(this.settings);
  }
  ...
  async updateSettings(newSettings: any): Promise<void> {
    ...
    for (const key of allowedKeys) {
      if (newSettings[key] !== undefined) {
        updatedSettings = mergeSettingsUpdate(updatedSettings, { [key]: newSettings[key] });
      }
    }
  ...
  async execute(req: ExecuteRequest) {
    ...
    switch (action) {
      ...
      case 'workflow': {
        ...
        return {
          content:
            typeof workflowResult === 'string' ? workflowResult : JSON.stringify(workflowResult),
