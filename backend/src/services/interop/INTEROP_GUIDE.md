...
interface ScheduleConfig {
  ...
  type: 'INGESTION' | 'WORKFLOW'; // v2 起统一为两类执行单元
  targetId: string; // 适配器名 (INGESTION) 或 工作流 ID (WORKFLOW)
  inputs?: {
    values?: Record<string, unknown>;
    bindings?: Record<
      string,
      { source: 'static' | 'variable'; value?: unknown; expression?: string }
    >;
  };
  enabled: boolean;
}
...
> 旧版业务化调度任务由后端在启动时自动迁移到上述新类型，新建任务请使用新值。
...
- `AI_PROVIDERS`: AI 模型提供商列表（类型：`GEMINI`, `CLAUDE`, `OPENAI`, `GLM`, `OLLAMA`）。
- `ADAPTERS`: 数据采集适配器列表。
- `PUBLISHERS`: 内容发布渠道列表。
- `STORAGES`: 资源存储配置列表。
