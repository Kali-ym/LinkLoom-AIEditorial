...
export class DatabaseMemoryService implements IMemoryService {
  ...
  async saveMemory(
    ...
    const existing = await this.store.findDuplicateMemoryHash(contentHash);
    if (existing) {
      LogService.info(`Memory content already exists in database: ${existing.id}, skipping save.`);
      return existing.id;
    }
    ...
    if (options.targetCategoryId) {
      await this.addToCategory(
        ...
        options.targetCategoryName || options.targetCategoryId,
        options.targetCategoryDescription || '日报跨日索引',
        entry,
        this.createEntrySummary(content)
      );
      return id;
    }
    ...
    try {
      ...
      const result = await this.agentService.runAgent(
        ...
        classifierPrompt,
        undefined,
        { silent: true, noTools: true, noSkills: true }
      );
      ...
      try {
        ...
        LogService.warn(
          `Memory classifier output not a valid JSON, falling back to uncategorized. Output: ${result.content}`
        );
      ...
      await this.addToCategory(
        ...
        entry,
        this.normalizeEntrySummary(decision.entrySummary, content)
      );

      LogService.info(`Memory saved and indexed in database: ${id} -> [${decision.categoryId}]`);
    } catch (error: any) {
      LogService.error(`Memory classification failed in database: ${error.message}`);
  ...
  private normalizeEntrySummary(summary: string | undefined, content: string): string {
    if (!summary || summary.trim().length < 12) {
      return this.createEntrySummary(content);
    }
    return summary.trim();
  }

  private createEntrySummary(content: string, maxLength = 260): string {
    const normalized = this.normalizeMemoryContent(content);
    const lines = normalized.split('\n');
    const meaningfulLines = lines.filter((line) => {
      const text = line.replace(/^[-*#\s]+/, '').trim();
      return text && !/^主题[:：]?$/i.test(text);
    });

    const preferred =
      meaningfulLines.find((line) => /核心结论|关键细节|结论|偏好|约束|规则/.test(line)) ||
      meaningfulLines[0] ||
      normalized;
  ...
  async queryMemory(
    ...
    if (!this.agentService) {
      return rawResults
        .map((r) => `[${getISODate(new Date(r.createdAt))}] ${r.content}`)
        .join('\n---\n');
    }
    ...
    const subAgentSystemPrompt = PromptService.getInstance().getPrompt('memory_query_subagent', {
      query
    });
  ...
  async mergeMemories(
    ...
    LogService.info(`Merging ${ids.length} memories in database mode...`);
    ...
    const result = await this.agentService.runAgent(MEMORY_WRITE_AGENT_ID, mergePrompt, undefined, {
      silent: false,
      noTools: true,
      noSkills: true
    });
    ...
    LogService.info(`Memories merged successfully in database. New ID: ${newId}`);
    return newId;
  }
  ...
  async getCategoryDetails(id: string): Promise<MemoryCategoryIndex | null> {
    ...
    return {
      ...
      entries: memories.map((m: any) => ({
        id: m.id,
        summary: this.normalizeEntrySummary(m.metadata?.summary, m.content),
