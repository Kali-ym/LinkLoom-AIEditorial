  ...
  async fetchAndTransform(config: any): Promise<UnifiedData[]> {
    ...
    try {
      ...
      if (this.enableTranslation && this.translationService) {
        LogService.info(
          `[Adapter: ${this.name}] Translation enabled, translating ${transformedData.length} items...`
        );
      ...
      LogService.info(
        `[Adapter: ${this.name}] Successfully fetched and transformed ${transformedData.length} items.`
      );
