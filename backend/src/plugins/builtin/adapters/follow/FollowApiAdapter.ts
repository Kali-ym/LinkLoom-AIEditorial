...
import { pickSourceImageFromFollowEntry } from '../../../../utils/sourceImage.js';
...
export class FollowApiAdapter extends BaseAdapter {
  ...
  /** 单次 POST 列表接口返回的最大条数（Folo API body.limit，默认 40） */
  private entryLimit: number = 40;
  ...
  constructor(
    ...
    this.entryLimit =
      typeof itemConfig.entryLimit === 'number' && itemConfig.entryLimit > 0
        ? Math.min(Math.floor(itemConfig.entryLimit), 500)
        : 40;
  ...
  async fetch(config: {
    ...
    LogService.info(
      `[FollowApiAdapter: ${this.name}] Requesting: ${config.apiUrl}, listId: ${listId || 'none'}, feedId: ${feedId || 'none'}, pages: ${fetchPages}, view: ${view}`
    );

    for (let i = 0; i < fetchPages; i++) {
      const body: any = {
        view: view,
        limit: config.entryLimit ?? this.entryLimit
      };
      if (view === 1) {
      ...
      try {
        ...
        if (!response.ok) {
          LogService.error(
            `[FollowApiAdapter: ${this.name}] Failed to fetch page ${i + 1}: ${response.status} ${response.statusText}`
          );
          break;
        }
        ...
        LogService.info(
          `[FollowApiAdapter: ${this.name}] Page ${i + 1} fetched, found ${pageData.length} entries.`
        );
        ...
        LogService.error(
          `[FollowApiAdapter: ${this.name}] Error fetching page ${i + 1}: ${error.message}`
        );
        break;
      }
    }
  ...
  private async fetchEntryDetail(
    id: string,
    config: { apiUrl: string; foloCookie?: string }
  ): Promise<string> {
    ...
    try {
      ...
      if (!response.ok) {
        LogService.error(
          `[FollowApiAdapter: ${this.name}] Failed to fetch detail for ${id}: ${response.status}`
        );
        return '';
      }
      ...
      LogService.error(
        `[FollowApiAdapter: ${this.name}] Error fetching detail for ${id}: ${error.message}`
      );
      return '';
    }
  }
  ...
  async transform(rawData: any, config?: any): Promise<UnifiedData[]> {
    ...
    for (const entry of filteredItems) {
      if (entry.entries?.id && !entry.entries.content) {
        LogService.info(
          `[FollowApiAdapter: ${this.name}] Fetching detail for entry: ${entry.entries.id}`
        );
      ...
      const sourceImage = pickSourceImageFromFollowEntry(entry);
      results.push({
        id: entry.entries.id,
