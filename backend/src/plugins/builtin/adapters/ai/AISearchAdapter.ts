...
export class AISearchAdapter extends BaseAdapter {
  ...
  async fetch(config: any): Promise<any[]> {
    ...
    LogService.info(
      `[AISearchAdapter: ${this.name}] Requesting ${executorId} for task: ${keyword}`
    );
    ...
    try {
      ...
      if (executorId.startsWith('workflow:')) {
        ...
        const agentId = executorId.startsWith('agent:')
          ? executorId.replace('agent:', '')
          : executorId;
      ...
      if (Array.isArray(data)) {
        ...
        LogService.warn(
          `[AISearchAdapter: ${this.name}] Response is not an array: ${JSON.stringify(data).slice(0, 100)}...`
        );
        return [];
      } else {
        LogService.error(
          `[AISearchAdapter: ${this.name}] Failed to parse response as JSON array from: ${content.slice(0, 200)}...`
        );
        return [];
      }
    } catch (error: any) {
  ...
  transform(rawData: any[], config?: any): UnifiedData[] {
    ...
    return rawData.map((item, index) => ({
      id: `ai-search-${this.name}-${index}-${Buffer.from(item.title || '')
        .toString('hex')
        .slice(0, 8)}`,
