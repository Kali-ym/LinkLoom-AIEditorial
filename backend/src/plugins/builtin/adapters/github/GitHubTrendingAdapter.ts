...
const ScraperConstants = {
  ...
  LANGUAGE_COLOR_SELECTOR:
    /<span class="repo-language-color" style="background-color:\s*(#[0-9a-fA-F]{3,6}|rgb\([^)]+\)|[a-zA-Z]+)">/s,
  TOTAL_STARS_SELECTOR:
    /<a [^>]*href="[^"]*\/stargazers"[^>]*>\s*(?:<svg.*?<\/svg>\s*)?([\d,]+)\s*<\/a>/s,
...
function tryRegexSequentially(
  text: string,
  regexArray: RegExp[],
  groupIndices: number | number[] = 1
): string | string[] | null {
  for (const regex of regexArray) {
    ...
    if (match) {
      ...
      return match[groupIndices] ? match[groupIndices].trim() : null;
    }
  }
...
export class GitHubTrendingAdapter extends BaseAdapter {
  ...
  private parseRepositoryArticle(articleHtml: string) {
    ...
    const repoLinkNameParts = tryRegexSequentially(
      articleHtml,
      ScraperConstants.REPO_LINK_NAME_SELECTORS,
      [1, 2, 3]
    ) as string[];
    ...
    const starsTodayStr = tryRegexSequentially(
      articleHtml,
      ScraperConstants.STARS_TODAY_SELECTORS,
      1
    ) as string;
  ...
  async fetch(config: {
    ...
    if (!response.ok) {
      LogService.error(
        `[GitHubTrendingAdapter: ${this.name}] Failed to fetch: ${response.status} ${response.statusText}`
      );
    ...
    if (repos.length === 0 && html.length > 0) {
      LogService.warn(
        `[GitHubTrendingAdapter: ${this.name}] No repositories parsed. Check selectors or URL: ${url}`
      );
    }

    LogService.info(
      `[GitHubTrendingAdapter: ${this.name}] Successfully fetched and parsed ${repos.length} items.`
    );
    return repos;
  }
