...
import { convertVideoToGif } from '../../../../utils/ffmpeg.js';
...
export class WechatService {
   ...
   */
  private static async runInQueue<T>(
    taskName: string,
    task: () => Promise<T>,
    ...
    LogService.info(
      `[Queue] "${taskName}" waiting. Active: ${this.activeProcessingCount}/${this.MAX_CONCURRENT_PROCESSING}, Mem: ${memBefore}MB`
    );
    ...
    LogService.info(
      `[Queue] "${taskName}" started. Mem: ${Math.round(process.memoryUsage().heapUsed / 1024 / 1024)}MB`
    );
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(
          () =>
            reject(new Error(`[Queue] Task "${taskName}" execution timeout after ${timeoutMs}ms`)),
          timeoutMs
        );
      });
      ...
      LogService.info(
        `[Queue] "${taskName}" success in ${duration}ms. Mem: ${memAfter}MB (Diff: ${memAfter - memBefore}MB)`
      );
      return result;
    } catch (err: any) {
  ...
  private async getProcessedImage(
    ...
    try {
      ...
      const result = await WechatService.runInQueue<ProcessedImageData>(
        `ProcessImg:${filename}`,
        async () => {
          ...
          if (
            detectedType === 'avif' ||
            detectedType === 'heif' ||
            (uploadType === 'body' && detectedType === 'webp')
          ) {
          ...
          if (processedBuffer.length > 2 * 1024 * 1024 && detectedType !== 'gif') {
            LogService.info(
              `[Sharp] Compressing large image (${(processedBuffer.length / 1024 / 1024).toFixed(2)}MB): ${filename}`
            );
      ...
      if (fileBuffer.length !== originalSize) {
        LogService.info(
          `[Sharp] Done: ${filename}. Size: ${(originalSize / 1024).toFixed(1)}KB -> ${(fileBuffer.length / 1024).toFixed(1)}KB`
        );
      }
    } catch (err) {
      LogService.warn(
        `Sharp process failed for ${filename}, using original. Error: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  ...
  private async uploadToWechat(
    ...
    const {
      ...
      filename,
      contentType
    } = preProcessed || (await this.getProcessedImage(imagePath, baseDir, uploadType));
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        if (i > 0) {
          LogService.info(
            `Retrying WeChat ${uploadType} upload (${i}/${retries}) for: ${imagePath}`
          );
  ...
  public async processHtmlImages(
    ...
    if (articleType === 'newspic') {
      ...
      for (const match of videoMatches) {
        ...
        const srcMatch =
          fullBlock.match(/(?:src|data-src)=["']([^"']+)["']/i) ||
        ...
        try {
          ...
          await WechatService.runInQueue(`Video2Gif:${hash.substring(0, 8)}`, async () => {
            LogService.info(
              `[Video] Starting FFmpeg conversion (limit threads: ${FFMPEG_THREADS})`
            );
            try {
          ...
          updatedHtml = updatedHtml.replace(
            fullBlock,
            `<img src="${resp.url}" style="width: 100%;" />`
          );
          if (resp.media_id) allMediaIds.push(resp.media_id);
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        ...
        if (options.articleType === 'newspic') {
          ...
          const finalImageMediaIds: string[] = [];
        ...
        if (response.data.errcode)
          throw new Error(
            `WeChat Publish error: ${response.data.errmsg} (${response.data.errcode})`
          );

        LogService.info(
          `Successfully published to WeChat draft. media_id: ${response.data.media_id}`
        );
