...
export class WechatPublisher implements IPublisher {
  ...
  async publish(
    ...
    if (!isHtml) {
      LogService.info(
        'Detected non-HTML content (Markdown), auto-rendering with WechatRenderer...'
      );
      finalContent = WechatRenderer.convert(content);
    }

    const displayDate = options.displayDate || getISODate().replace(/-/g, '/');
    const displaySummary = options.digest || '';
    ...
    // 处理图片 (上传到微信)
    const fallbackLogo = this.config.fallbackLogoUrl || '';
    const {
      html: processedHtml,
      firstMediaId,
      allMediaIds
    } = await this.service.processHtmlImages(finalContent, undefined, fallbackLogo, articleType);
